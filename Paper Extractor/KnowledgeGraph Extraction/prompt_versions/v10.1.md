...
- `readout`: `null` (no experimental readout for citations)
- `perturbation`: `null` (no perturbation for citations)
- `approach`: `"citation_reference"`

**All evidence fields must be present in citation stubs** — set non-applicable fields to `null` explicitly rather than omitting them. An evidence unit with missing fields (vs. null fields) is flagged as incomplete by the validation pipeline.
...
2. **Evidence depth (CRITICAL — verify before outputting)**: After writing all claims, compute: *sum of all evidence_links entries across all claims ÷ total claim count*. This ratio MUST be ≥2.0. If it falls below 2.0, revisit each claim with only a single evidence link and ask: does the paper show this same finding via a second experiment, replicate, complementary assay, or additional figure panel? If yes, add the second link. A single evidence link is acceptable only when the paper presents exactly one experiment for that specific claim and no other figure panel or assay corroborates it.
...
9. **`quantitative_context` (CRITICAL — check every claim before finalizing)**: ANY claim where the surrounding results text reports a number — fold-change, percentage, cell count, expression intensity, frequency, concentration, dose, or timepoint — MUST have `quantitative_context` populated with that number. The checklist question is: *does this claim's supporting text contain ANY numerical value?* If yes, populate. Set `quantitative_context: null` ONLY for purely conceptual or categorical claims where zero numbers appear anywhere in the surrounding results text.
