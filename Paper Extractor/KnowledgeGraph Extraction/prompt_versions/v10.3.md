...
Process the paper in three sequential passes before writing JSON:

**Pass 1 — Evidence inventory**: Scan every "Fig.", "Figure", "Table", and supplementary reference in the paper. List each distinct experiment and figure panel. Each item in this inventory becomes one evidence unit.

**Pass 2 — Write all evidence units first**: Create every evidence unit from your Pass 1 inventory before writing any claims.

**Pass 3 — Write claims, then multi-link**: Write each claim, then scan your COMPLETE evidence list and add EVERY unit that bears on this claim to `evidence_links`. If Figures 2A and 2D both validate the same finding, both evidence IDs appear in the same claim's `evidence_links`. Do NOT stop after linking the first matching evidence unit — one claim can and should reference multiple evidence units when the paper provides multiple supporting experiments.
...
**Rule — scan before setting null**: For each claim, read the 1-3 sentences in the paper where this finding is stated. Does any number appear — %, fold-change, p-value, n=, dose, timepoint, concentration, accuracy, score, or distance metric? If YES → populate `quantitative_context`. Use free-text in the most appropriate field for values that do not fit standard fields (e.g., `"concentration": "1.5-fold upregulated"`, `"timepoint": "30% reduction at 48h"`, `"dose": "AUC = 0.92"`). Set `null` ONLY after completing this scan and confirming NO number appears anywhere near this finding. **Default to populating, not skipping.** Every quantitative measurement reported alongside a claim must appear somewhere in `quantitative_context`.
