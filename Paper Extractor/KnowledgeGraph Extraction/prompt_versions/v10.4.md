...
- One evidence unit per distinct experiment or figure panel (for experimental evidence) or per distinct cited finding (for citation evidence stubs). Evidence units are NOT claim-exclusive — a single figure panel demonstrating multiple biological relationships should appear in the `evidence_links` of ALL claims it supports, using the same `evidence_id` in each. Do NOT create a new evidence unit to avoid reusing an existing one.
...
2. Every `primary_empirical` claim links to at least TWO evidence units where the paper provides multiple supporting experiments or figure panels (e.g., both Western blot and immunofluorescence confirm the same finding, or two figure panels independently demonstrate the same result). A single evidence link is acceptable only when the paper has NO other experiment — direct or indirect, including orthogonal readouts, dose-response panels, rescue experiments, or replicate conditions — that independently corroborates the same claim. **Target ≥2 evidence links per claim on average across the full extraction.**
