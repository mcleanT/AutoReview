...
- One evidence unit per distinct experiment or figure panel (for experimental evidence) or per distinct cited finding (for citation evidence stubs). **The claim-to-evidence relationship is many-to-many, not 1:1.** A single claim should reference ALL experiments that test it — typically 2–4 links for a central finding supported by multiple panels or assays. A single evidence unit may be referenced by multiple claims. Do NOT assume each claim maps to exactly one evidence unit, or that a claim with one link is complete.
...
- **`result_summary` is REQUIRED**: State what the experiment CONCLUDED, not what was done. It must be a complete sentence that a reader could evaluate as true or false.
- **`readout` is REQUIRED** for all experimental evidence (non-citation stubs): Name the specific molecular or cellular measurement (e.g., "T/Brachyury mRNA level by qRT-PCR", "phospho-Smad2 band density by Western blot", "differentially expressed gene count by RNA-seq"). **`readout` is NOT the same as `description`** — `description` says how the experiment was performed; `readout` names what was measured. Do NOT leave `readout` blank on the assumption that `description` already covers it. Example: "BMP4 knockout caused 3.2-fold reduction in T/Brachyury, confirming BMP4 is required for mesoderm specification" — NOT "BMP4 knockout gastruloids assessed by immunofluorescence."
