...
**Rule:** For every `primary_empirical` claim, **copy the quantitative experimental conditions directly from the figure legend or methods into this field**: the treatment concentration, the timepoint(s) at which the result was measured, and the dose. Do not leave `quantitative_context` null if the figure legend or experimental description contains any numbers. Set `null` only for `attributed_prior` claims and purely conceptual claims with no associated experiment. For fold-changes and percentages that do not fit `concentration`/`dose`/`timepoint`, record them as free-text in the most appropriate field (e.g., `"concentration": "1.5-fold upregulated"`, `"timepoint": "30% reduction at 48h"`). Every number that defines the experimental conditions of a `primary_empirical` claim must appear in `quantitative_context`.
...
```json
{
  ...
  "evidence": [
    ...
    {
      "evidence_id": "e_002",
      "description": "BMP4 knockout gastruloids stained for Tbx6 and Sox17 to assess paraxial mesoderm and endoderm lineages at day 5",
      "result_summary": "BMP4 knockout abolished Tbx6+ paraxial mesoderm (0% vs 28% of cells in controls) and reduced Sox17+ endoderm (3% vs 15%), confirming BMP4 is required for both mesodermal and endodermal specification",
      "model_system": "mouse ESC gastruloids (E14Tg2a)",
      "organism": "Mus musculus",
      "perturbation": "CRISPR KO of BMP4",
      "readout": "Tbx6 and Sox17 immunofluorescence, cell-type frequency",
      "result_direction": "positive",
      "effect_size": "Tbx6: 0% vs 28%; Sox17: 3% vs 15%",
      "p_value": "p < 0.001",
      "sample_size": "n=30 gastruloids per condition",
      "key_figure": "Figure 2B",
      "approach": "cell_biology",
      "assay_types": ["immunofluorescence", "confocal_microscopy"],
      "evidence_strength": "direct_experimental"
    },
