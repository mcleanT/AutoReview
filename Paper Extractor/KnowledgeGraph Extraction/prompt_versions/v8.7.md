...
**Rule:** Extract `quantitative_context` for ALL claim types whenever the paper reports ANY associated number — fold-changes, percentages, cell counts, expression intensities, frequencies, concentrations, doses, or timepoints. **Default to populating, not skipping.** Set `null` ONLY when the claim is a completely general statement with no numerical values reported anywhere in the surrounding results text. For fold-changes and percentages that do not fit `concentration`/`dose`/`timepoint`, record them as free-text in the most appropriate field (e.g., `"concentration": "1.5-fold upregulated"`, `"timepoint": "30% reduction at 48h"`). Every quantitative measurement reported alongside a claim must appear somewhere in `quantitative_context`.
