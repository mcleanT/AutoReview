...
**Rule:** Extract `quantitative_context` for ALL claim types — not just mechanistic_causal and comparative. If a correlational or existence claim is qualified by concentration or timepoint, capture it. Set `null` only if the claim is genuinely general/unqualified.
