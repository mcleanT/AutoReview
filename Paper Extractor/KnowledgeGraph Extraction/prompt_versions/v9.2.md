...
| `regulates` | X controls Y — **LAST RESORT ONLY**: use ONLY when direction is genuinely unknown AND the paper provides no evidence for net activation or inhibition. If you can commit to `induces` or `inhibits`, you MUST do so. |

**Counter-example — WRONG (regulates as generic fallback):**
- "FGF signaling regulates mesodermal gene expression" → paper shows FGF activates Tbx6 → use `induces`
- "Loss of Rai14 regulates macropinocytosis" → paper shows knockdown reduces it → use `is_required_for`
- "BMP4 regulates cell fate" → paper shows BMP4 promotes mesoderm → use `induces`

**CORRECT use of `regulates`:** "Drug X regulates Wnt pathway activity" when the paper reports that X increases Wnt in some conditions and decreases it in others, with no net directional conclusion possible. If only one direction is shown, use `induces` or `inhibits`.
...
**Counter-example — WRONG (omitting quantitative_context when numbers exist in paper):**
Paper text: "Treatment with 10 ng/mL BMP4 for 48h increased T/Brachyury expression 3.2-fold."
Extracted: `"natural_language": "BMP4 increases T/Brachyury expression", "quantitative_context": null` — INCORRECT. The dose and timepoint define the scope of this claim; omitting them makes cross-paper comparison impossible.

**CORRECT:** `"quantitative_context": {"concentration": "10 ng/mL BMP4", "timepoint": "48h", "dose": null}`

**Decision rule:** If ANY numeric qualifier (dose, concentration, timepoint, passage number, cell density, fold-change threshold) appears in the paper for this finding, populate `quantitative_context`. Treat it as REQUIRED for dose/time-qualified claims — `null` is only valid when the paper makes a genuinely unqualified general statement with no numbers attached.
...
2. Every `primary_empirical` claim links to at least **two evidence units** when the paper presents the same finding from multiple independent experiments. A single claim validated by both immunofluorescence AND western blot, or by both a knockout AND a rescue experiment, must reference ALL supporting evidence units in its `evidence_links` array.

**Counter-example — WRONG (single link despite multiple experiments):**
Paper shows: Figure 2A (immunofluorescence) AND Figure 2B (qPCR) both confirm BMP4 loss reduces T/Brachyury. Extracted claim has only `evidence_links: [{"evidence_id": "e_001", "direction": "supports"}]` — Figure 2B is silently dropped.

**CORRECT:** Both experiments are separate evidence units (e_001 and e_002), and both appear in `evidence_links`: `[{"evidence_id": "e_001", "direction": "supports"}, {"evidence_id": "e_002", "direction": "supports"}]`

**Rule:** After drafting all evidence units, cross-check each claim: does its evidence_links list reflect EVERY experiment in the paper that directly tests that relationship? Add missing links before finalizing.
