...
| `regulates` | X controls Y where direction is **genuinely ambiguous** in the paper. **MANDATORY STOP CHECK before use**: (1) Does the paper show X increases/activates/induces Y? → use `induces`. (2) Does the paper show X decreases/blocks/suppresses Y? → use `inhibits`. Only if neither applies, use `regulates`. Hard cap: `regulates` MUST appear in fewer than 20% of all claims. If you are using it more than that, you are treating it as a generic fallback — that is an extraction error. |
...
**Rule — HARD CONSTRAINT:** Extract `quantitative_context` for ALL claim types. ANY claim where the paper reports a specific concentration, dose, timepoint, duration, fold-change, percentage, passage number, or cell count associated with that claim MUST have non-null `quantitative_context`. Default behavior: populate it. Set `null` ONLY for purely definitional or conceptual claims where zero quantitative parameters appear in the paper for that claim. Do NOT leave `quantitative_context` null on attributed_prior, correlational, or existence claims merely because they are not mechanistic — if numbers are present in the paper for that claim, you MUST populate the field. Leaving `quantitative_context` null when the paper provides numbers is an extraction error.
...
2. Every `primary_empirical` claim MUST link to at least two evidence units when the paper provides multiple supporting experiments (different figure panels, different assays, replicate experiments, or orthogonal methods confirming the same finding). After drafting all claims, audit each claim's `evidence_links`: if two or more figure panels or experiments in the paper are relevant to the same claim, all of them MUST be listed. Target: average ≥2 `evidence_links` per claim. A single evidence link is acceptable ONLY when the paper contains exactly one experiment supporting that claim — this MUST be the exception, not the default.
