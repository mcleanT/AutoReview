...
def nli_score(
    ...
    html: bool = typer.Option(True, help="Generate interactive HTML"),
    html_output: str | None = typer.Option(None, help="Custom HTML output path"),
    json_report: bool = typer.Option(
        True, "--json-report/--no-json-report", help="Save JSON report"
    ),
) -> None:
    """Score knowledge graph claims with cross-claim NLI contradiction detection."""
    from autoreview.knowledge_graph import NLIConfig, classify_cross_claims, load_graph, save_graph

    typer.echo(f"Loading graph from {input_path}...")
    ...
    config = NLIConfig(
        ...
        filter_parallel_assertions=not no_parallel_filter,
        use_predicate_opposition=not no_predicate_opposition,
    )

    typer.echo("Running cross-claim NLI...")
    result = classify_cross_claims(graph, config)

    typer.echo(f"  Total pairs: {result.total_pairs}")
    typer.echo(f"  Parallel skipped: {result.parallel_skipped}")
    typer.echo(f"  Structural resolved: {result.structural_resolved}")
    typer.echo(f"  NLI classified: {result.nli_classified}")
    typer.echo(f"  Contradictions (p>0.5): {result.contradictions_p05}")
    typer.echo(f"  Claims updated: {result.claims_updated}")

    # Determine output path
    if output is None:
        output = str(input_path).replace(".pkl", "_nli")
    output_path = Path(output)

    save_graph(graph, output_path)
    typer.echo(f"Saved: {output_path}.pkl and {output_path}.graphml")

    if html:
        from autoreview.knowledge_graph.interactive import generate_interactive_html

        html_path = Path(html_output) if html_output else output_path.parent / "interactive_kg.html"
        generate_interactive_html(graph, html_path)
    ...
    if json_report:
        ...
        report = {
            ...
            "parallel_skipped": result.parallel_skipped,
            "structural_resolved": result.structural_resolved,
            "nli_classified": result.nli_classified,
            "contradictions_p05": result.contradictions_p05,
            "contradictions_p08": result.contradictions_p08,
            "claims_updated": result.claims_updated,
            "top_contradictions": [
                {
                    "claim_a_id": pr.claim_a_id,
                    "claim_b_id": pr.claim_b_id,
                    "p_contradiction": pr.p_contradiction,
                    "p_entailment": pr.p_entailment,
                    "p_neutral": pr.p_neutral,
                    "method": pr.method,
                    "shared_entities": pr.shared_entities,
                    "context_mismatch": pr.context_mismatch,
                    "original_p_contradiction": pr.original_p_contradiction,
                }
                for pr in sorted(
                    result.pair_results, key=lambda x: x.p_contradiction, reverse=True
                )[:100]
            ],
        }
...
def nli_diagnose(
    ...
    from autoreview.knowledge_graph import NLIConfig, diagnose_evidence_directions, load_graph

    graph = load_graph(input_path)
    config = NLIConfig(device=device)
    result = diagnose_evidence_directions(graph, config)

    typer.echo(f"Classified {result.total_pairs} evidence-claim pairs")
    for label, count in sorted(result.label_distribution.items()):
        pct = count / result.total_pairs * 100 if result.total_pairs else 0
        typer.echo(f"  {label}: {count} ({pct:.1f}%)")

    if output is None:
        output = str(input_path).replace(".pkl", "_nli_diagnostic.json")
    Path(output).write_text(json.dumps(result.results, indent=2))
