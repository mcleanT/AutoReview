...
nodes = [
    ...
    ("Thematic Clustering\n+ Contradiction Detection",
                                       MAIN_X,  9.35,  W,    H,   ANALYSIS_COLOR),
...
# Title
ax.text(5.5, 13.75, "AutoReview Pipeline", ha="center", va="center",
...
def draw_node(ax, n):
    ...
    ax.text(x, y, label, ha="center", va="center",
            fontsize=7.8, color=TEXT_LIGHT, fontweight="bold",
...
for label, y_top, y_bot, color in phases:
    mid_y = (y_top + y_bot) / 2
    ax.text(-0.35, mid_y, label, ha="center", va="center",
            fontsize=6.5, fontweight="bold", color=color,
            rotation=90, fontstyle="italic", alpha=0.7)
...
arrow(ax, n_out[1] + n_out[3]/2, n_out[2],
          n_oc[1]  - n_oc[3]/2,  n_oc[2],  color=CRITIQUE_COLOR)
curved_arrow(ax, n_oc[1] - n_oc[3]/2, n_oc[2] - 0.12,
                 n_out[1] + n_out[3]/2, n_out[2] - 0.12,
                 color=LOOP_ARROW, rad=-0.25)
...
arrow(ax, n_sw[1] + n_sw[3]/2, n_sw[2],
          n_sc[1]  - n_sc[3]/2,  n_sc[2],  color=CRITIQUE_COLOR)
curved_arrow(ax, n_sc[1] - n_sc[3]/2, n_sc[2] - 0.12,
                 n_sw[1] + n_sw[3]/2, n_sw[2] - 0.12,
                 color=LOOP_ARROW, rad=-0.25)
...
arrow(ax, n_ad[1] + n_ad[3]/2, n_ad[2],
          n_hc[1]  - n_hc[3]/2,  n_hc[2],  color=CRITIQUE_COLOR)
curved_arrow(ax, n_hc[1] - n_hc[3]/2, n_hc[2] - 0.12,
                 n_ad[1] + n_ad[3]/2, n_ad[2] - 0.12,
                 color=LOOP_ARROW, rad=-0.25)
...
arrow(ax, n_qe[1] - n_qe[3]/2, n_qe[2],
          n_qcc[1]  + n_qcc[3]/2, n_qcc[2],
          color=CHECK_COLOR, lw=1.2, ls="--")
...
arrow(ax, n_sd[1]  - n_sd[3]/2,  n_sd[2],
          n_cac[1] + n_cac[3]/2, n_cac[2],
          color=CHECK_COLOR, lw=1.2, ls="--")
...
arrow(ax, n_gs[1]  - n_gs[3]/2,  n_gs[2],
          n_pgr[1] + n_pgr[3]/2, n_pgr[2],
          color=CHECK_COLOR, lw=1.2, ls="--")
...
ax.text(n_gs_node[1], n_gs_node[2] - n_gs_node[4]/2 - 0.13,
        "conditional", ha="center", va="top",
        fontsize=6.5, color=SEARCH_COLOR, style="italic")

# ── Enrichment annotation ────────────────────────────────────────────────────
n_ce = next(n for n in nodes if n[0] == "Contextual Enrichment")
ax.text(n_ce[1] + n_ce[3]/2 + 0.15, n_ce[2],
        "adjacent material\n(background, cross-field,\nclinical implications)",
        ha="left", va="center",
        fontsize=6.0, color=ENRICH_COLOR, style="italic")

# ── Passage search annotation ────────────────────────────────────────────────
n_ps = next(n for n in nodes if n[0].startswith("Passage Search"))
ax.text(n_ps[1] + n_ps[3]/2 + 0.15, n_ps[2],
        "mine undercited claims,\nretrieve + revise sections",
        ha="left", va="center",
        fontsize=6.0, color=SEARCH_COLOR, style="italic")
