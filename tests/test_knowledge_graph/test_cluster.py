...
class TestFormFindings:
    ...
    def test_deterministic_finding_ids(self):
        G = _make_cluster_graph()
        clusters = build_topic_clusters(G)
        findings1 = form_findings(clusters, G)
...
class TestDetectFindingContradictions:
    def _get_activating_findings(self, G=None):
        if G is None:
            G = _make_cluster_graph()
        clusters = build_topic_clusters(G)
        findings = form_findings(clusters, G)
    ...
    def test_directional_type_when_coupling_high(self):
        ...
        G = nx.MultiDiGraph()
        G.add_node("s", canonical_name="A")
        G.add_node("o", canonical_name="B")
        G.add_edge(
            "s",
            "o",
            edge_id="x1",
...
class TestInterpretiveConflictWithGraph:
    ...
    def test_interpretive_detected_with_graph(self):
        G = self._make_interpretive_graph(same_paper=False)
        clusters = build_topic_clusters(G)
        findings = form_findings(clusters, G)
    ...
    def test_interpretive_not_detected_same_paper(self):
        ...
        G = self._make_interpretive_graph(same_paper=True)
        clusters = build_topic_clusters(G)
        findings = form_findings(clusters, G)
        contradictions = detect_finding_contradictions(findings, clusters, graph=G)
        interpretive = [c for c in contradictions if c.contradiction_type == "interpretive"]
