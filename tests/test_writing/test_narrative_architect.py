...
def _make_directive(section_id: str = "1") -> SectionNarrativeDirective:
    return SectionNarrativeDirective(
        ...
        narrative_role=(
            "Sets up the central tension between mechanistic and epidemiological evidence"
        ),
        central_claim="The gut-brain axis plays a causal role in neurodegeneration",
        structural_suggestion=(
            "problem-solution: establish the unknown, then trace how the field addressed it"
        ),
        key_insights=[
        ...
        transition_to_next=(
            "These findings set the stage for the contradictions examined in the next section"
        ),
    )
...
def _make_narrative_plan(outline: ReviewOutline) -> NarrativePlan:
    ...
    return NarrativePlan(
        central_argument=(
            "The gut-brain axis literature reveals a causal mechanism for neurodegeneration"
        ),
        narrative_arc=(
...
class TestNarrativeModels:
    ...
    def test_narrative_plan_serialization_roundtrip(self):
        ...
        for orig, rest in zip(plan.section_directives, restored.section_directives, strict=False):
...
class TestSectionWriterDirectiveIntegration:
    ...
    async def test_write_all_sections_without_plan(self):
        ...
        for _sid, draft in drafts.items():
            assert draft.text
